...
OUT=BASE/'work/swift64/url-consumers'
PINS=json.loads((BASE/'work/swift64/published-source.json').read_text())
...
'''.replace('URL',json.dumps(url)).replace('REV',json.dumps(pin)).replace('MODULE',json.dumps(name))
 ...
 if r.returncode==0:
  ...
  original=json.loads((BASE/'work/swift64/candidate'/name/'report.json').read_text())['source_files_sha256']
