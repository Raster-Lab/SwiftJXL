...
OUT=BASE/'work/swift64/benchmark-repeat'
OUT.mkdir(parents=True,exist_ok=True)
SOURCES={'baseline':BASE/'work/swift64/baseline/SwiftJ2K/source','candidate':BASE/'outputs/SwiftJ2K'}
...
let package = Package(name: "StorageBench", platforms: [.macOS("26.0")],
...
builds=json.loads((BASE/'work/swift64/benchmark/builds.json').read_text())
...
result=dict(method='Same-host release executables; Repeated investigation of initial 64x63 write +6.46%; same existing release executables, 5 warmups then 20 interleaved measurements per case; 5% median/peak-memory regression investigation threshold set by contract PERF-03 before execution. Shared Image.swift evaluated through SwiftJ2K, not a codec-throughput or multi-platform claim.',
